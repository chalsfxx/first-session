...
console.log("-------------------");
console.log(
  `La Palabra mas larga dentro de este array es:  "${findLongestWord(words)}"`
);
// V2 - K
...
console.log("-------------------");
console.log(
  `La Palabra mas larga dentro de este array es:  "${findLongestWord3(words)}"`
);
...
console.log(
  `For LOOP - La suma de todos estos numeros es: "${sumNumbers(numbers)}"`
);
...
console.log(
  `REDUCE V1 - La suma de todos estos numeros es: "${sumNumbers2(numbers)}"`
);
...
console.log(
  `FOR EACH - La suma de todos estos numeros es: "${sumNumbers3(numbers)}"`
);
...
console.log(
  `Version 002 - La suma de este array generico es de un total de: ${sum2(
    mixedArr
  )}`
);
...
// V2 - A - SIMPLIFIED TERNARIOS & ARROW
const averageWordLength2 = (wordsArr) =>
  !wordsArr.length ? null : wordsArr.join("").length / wordsArr.length;
console.log(
  `TERNARIO - ARROW - La media de palabras en este array es: ${averageWordLength2(
    wordsArr
  )}`
);
// Bonus - Iteration #4.1
...
const string = 'Hola soy una string'
if (!string) console.log('string NO existe')


/* 
MISMA FUNCIÓN QUE EN LA LÍNEA 239
function avg2(arr) {
  if (arr.length === 0) {
    return null;
  } else {
